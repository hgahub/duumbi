...
## Milestone 7 — Registry & Distribution (8–10 hét)

> **Cél:** "crates.io a szemantikus gráfoknak" — modulok megosztása, binary cache, determinisztikus build.
> **Kill Criterion:** Ugyanaz a modul két gépen (azonos target) ugyanazt a semantic hash-t adja. `duumbi publish` + `duumbi deps install` működik egy publikus modulra, és a lockfile determinisztikus.

### Deliverable-ök

- [ ] **M7-REG-01**: Registry szerver (axum) — `registry.duumbi.dev` API
- [ ] **M7-REG-02**: `duumbi publish` — modul feltöltés (JSON-LD + metadata + SemVer)
- [ ] **M7-REG-03**: `duumbi deps install` — registry-ből letöltés + lockfile update
- [ ] **M7-REG-04**: `duumbi deps add <module>` — dependency hozzáadása
- [ ] **M7-REG-05**: `duumbi deps update` — lock file frissítése
- [ ] **M7-REG-06**: `duumbi search "<query>"` — típus és tag alapú keresés
- [ ] **M7-REG-07**: Semantic hash — gráf struktúra + op értékek ujjlenyomata (@id nélkül)
- [ ] **M7-REG-08**: Binary cache — content-addressable `.o` fájlok CDN-ről
- [ ] **M7-REG-09**: `duumbi yank <version>` — hibás verziók visszavonása
- [ ] **M7-REG-10**: Lockfile v1 — teljes dependency fa rögzítés, registry URL-ek
- [ ] **M7-REG-11**: Registry web frontend (Leptos) — böngészés, keresés, modul docs
- [ ] **M7-REG-12**: Studio integráció — registry keresés a sidebar-ból
...
| Szál | Feladat szám | Becsült effort |
|------|-------------|----------------|
| Registry backend | 6 feladat | 4–5 hét |
| CLI parancsok | 4 feladat | 2 hét |
| Binary cache | 2 feladat | 2 hét |
| Web frontend | 2 feladat | 2 hét |
| **Összesen** | **12 feladat** | **8–10 hét** |
...
Phase 3 befejezés (2026 március)
    ...
    │   Kill: determinisztikus hash + publish+install működik
    │
...
Összesen: ~40–50 hét (10–12 hónap)
```
