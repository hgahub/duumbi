...
Acceptance: [5704193547](https://github.com/hgahub/duumbi/issues/817#issuecomment-5704193547).
...
- Review corrected the evidence threshold: official model capability plus an
  ...
  support letter for every model is not required. No scope was removed.
...
| S7-04: Provider-by-provider release would conflict with joint delivery. | One unit owns all nine criteria; ten named models stay in scope, with no new sub-issues. |
...
invariants appear in AC-817-01–09 and BDD-817-01–13. No new product decision is
...
| S9-09: Haiku/Fable/Opus controls are not interchangeable. | Separate explicit-none encoding and effort cells; no Haiku budget mapping; automatic Fable tool choice. |
...
| S9-12: Required validation must use the actual production path and all ten models. | 140-cell offline matrix plus opt-in joint live tool/answer/callback/capture coverage; no passing result for skipped/denied rows. |
