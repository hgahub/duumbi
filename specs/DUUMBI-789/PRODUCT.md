...
**Ready for Build** after a Ready-for-Build handoff or after an accidental Done
/ reopen, without using the GitHub Project board.
...
2. A distinct reopen / accidental-Done correction Slack message includes the
   same Project Status buttons when an execution issue is reopened after close
   and still belongs on the Ready-for-Build track.
...
4. Combined two-file merged spec PRs do not block the status buttons.
5. **Undo Done** moves Project Status on **open** issues. Slack does not reopen
   closed issues.
6. Failures reply in the Slack thread with a Stage Approval / Project fallback
   link so the Owner can finish the correction in GitHub.
7. GitHub remains the source of truth. Slack is an action surface, not a second
...
1. Add Slack Block Kit buttons to `.github/workflows/ready-for-build-handoff.yml`
   posts.
2. Add a reopen / accidental-Done correction Slack message with the same
   buttons when a previously closed execution issue is reopened and still has
   Ready-for-Build signals (`tech-spec-approved` and/or a prior Ready-for-Build
   handoff marker, or Project Status **Done** after reopen).
3. Route those buttons through `scripts/slack-approval-bridge` with a new
   Project Status-only `action_type` and `repository_dispatch` event.
4. Execute the click in a Project Status-only GitHub Actions path that updates
   Project V2 Status and does not run spec-PR merge validation.
5. Record a durable GitHub issue comment for the Project Status decision.
6. Reply in Slack on success and on failure, including a fallback link.
7. Document the bridge dispatch table, fallback workflow, and Azure Function
   deploy/rollback notes.
8. Add focused tests for bridge routing, workflow contracts, and handoff
   payload shape.
...
- The bridge does not forward Slack `response_url` in `repository_dispatch`
  payloads. Immediate Slack follow-up stays in the Function process.
- `ready-for-build-handoff.yml` is text-only today and is idempotent per issue
  via the v1 marker comment.
...
- Project V2 updates in existing workflows use `GH_PROJECT_PAT` and GraphQL
  `updateProjectV2ItemFieldValue` on the Status single-select field.
- No current workflow posts a Slack message on `issues.reopened`.

Assumptions:

- The review channel (`SLACK_REVIEW_CHANNEL_ID`) is the correct surface. v1
  does not add a second channel.
- Existing secrets (`SLACK_BOT_TOKEN`, `GH_PROJECT_PAT`, Function
  `GITHUB_TOKEN` / `SLACK_SIGNING_SECRET`) are sufficient. No new secret names
  are required.
- Project Status option names **Ready for Build** and **Done** already exist.
  This issue does not create them.
...
| Messages that get buttons | Ready-for-Build handoff **and** reopen / accidental-Done correction | Stage 5 remaining question; #779 marker already consumed after reopen |
...
1. **Ready-for-Build handoff** (`.github/workflows/ready-for-build-handoff.yml`)
   must post Block Kit `blocks` in addition to fallback `text`. Buttons appear
   on every new handoff that the workflow posts.
2. **Reopen / accidental-Done correction** must post a **new** Slack message
   when an execution issue is reopened and still has Ready-for-Build track
   signals. A prior Ready-for-Build marker must not suppress this correction
   message. Use a distinct marker, for example
   `<!-- duumbi-reopen-project-status-slack-notified:v1 issue=N -->`.
3. Other existing Slack cards (Stage 5/7/9/10/11/12) keep their current
   buttons and copy. This issue does not add Project Status buttons to Stage 7
   or Stage 9 approval cards.

### Buttons

Primary button, always present on those two messages:
...
3. GitHub runs the Project Status-only workflow.
4. The workflow loads the issue. If the issue is **closed**, it fails closed:
   no Project update, no reopen. Slack receives a failure reply telling the
   Owner to reopen the GitHub issue first, then use the correction message or
   Project UI.
5. If the issue is **open**, the workflow sets Project V2 Status to
   **Ready for Build** using `GH_PROJECT_PAT`.
6. The workflow writes an issue comment recording reviewer, decision, previous
   status when known, new status, and that no spec PR was merged.
7. Slack receives a success reply in the same thread.

Idempotency: if Status is already **Ready for Build**, treat the click as
success and say so in Slack. Do not fail.

### Failure Slack fallback

If signature verification fails, dispatch fails, `GH_PROJECT_PAT` is missing,
the Status option is missing, or GraphQL update fails, Slack must receive a
non-replacing follow-up that includes:
...
Feature: Slack Project Status buttons for Ready for Build
  ...
  Scenario: Owner sets Ready for Build from a Ready-for-Build handoff
    Given an open GitHub issue whose Project Status is Done or Spec Needed
    ...
    And GitHub Project V2 Status for that issue becomes Ready for Build
  ...
  Scenario: Combined two-file spec PR does not block the status button
    Given the issue reached Ready-for-Build track through a merged combined
      PRODUCT.md and TECHNICAL.md spec PR
    And Stage Approval merge automation did not update Project Status
  ...
  Scenario: Undo Done moves Status on an open issue only
    Given an open GitHub issue whose Project Status is Done
    And a Ready-for-Build or reopen correction Slack message shows Undo Done
    When the Owner confirms Undo Done
    Then Project Status becomes Ready for Build
  ...
  Scenario: Closed issue is not reopened from Slack
    ...
    When a Project Status button is clicked for that issue
    Then Project Status is not changed
    And the issue remains closed
    And Slack receives a failure reply telling the Owner to reopen the issue
      in GitHub first, then use Project UI or the reopen correction message

  Scenario: Reopen after accidental Done gets a correction message
    Given an execution issue was closed and Project Status moved to Done
  ...
  Scenario: Dispatch or Project update failure keeps a Slack fallback
    Given the Owner clicked Set Ready for Build
    When GitHub dispatch or Project V2 update fails
    Then Slack receives a non-replacing warning
  ...
  Scenario: Already Ready for Build is idempotent success
    Given an open issue whose Project Status is already Ready for Build
    When the Owner clicks Set Ready for Build
...
2. Add the Project Status-only GitHub Actions workflow and `workflow_dispatch`
   fallback.
3. Add Block Kit buttons to Ready-for-Build handoff Slack posts.
4. Add reopen / accidental-Done correction Slack posts with a distinct marker.
5. Record GitHub issue comments and Slack success/failure replies.
6. Update bridge README and orchestration docs.
7. Add bridge unit tests and workflow contract tests.
8. Document Azure Function deploy and rollback.

Tasks 1–2 can proceed independently of 3–4 once the payload contract is
stable. Task 7 should land with each slice, not only at the end.
...
1. A Ready-for-Build Slack handoff for an open issue includes **Set Ready for
   Build** and, when Status is Done or unknown, **Undo Done**.
2. Clicking **Set Ready for Build** on an open issue sets Project V2 Status to
   **Ready for Build** without merging any PR.
...
4. A combined two-file spec PR history does not block either button.
5. A closed issue click does not reopen the issue and replies in Slack with
   the GitHub reopen + Project fallback.
6. Dispatch or Project update failure replies in Slack with the Project
   Status-only workflow link and a Project UI reminder.
...
8. Reopening a Ready-for-Build-track issue posts a correction Slack message
   even when the v1 Ready-for-Build marker already exists.
9. Bridge tests prove `project_status` routes to `project-status` and that
   Stage 5/7/9/10 routing is unchanged.
10. Azure Function deploy/rollback notes exist in the bridge README.
